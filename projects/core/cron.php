...
//                print_r($urls);
                foreach ($urls as $url)
                {
                    ...
                    $runlog =wpusm_post_log_create($url);
                    if($runlog == 2)
                    {
                        continue;
                    }
                    ...
                    if(wpusm_check_url_exist($url_base))
                    {
                        echo "<div><h3 style='color: red'>{$url} - Exist !</h3></div>";
                    }else{
                        $app->Weburl = $url;
...
function wpusm_cron_single_work($app){
...
// exit();
        ...
        $post_id = wpusm_Postwp($app->Title, $mask . $app->PostThumb, $app->Content, $url, $app->Categorys, $app->Tags);
